...
namespace Lines202605\TomasVotruba\Lines\FeatureCounter\ValueObject;

use Lines202605\PhpParser\Modifiers;
use Lines202605\PhpParser\Node;
use Lines202605\PhpParser\Node\Arg;
use Lines202605\PhpParser\Node\AttributeGroup;
use Lines202605\PhpParser\Node\Expr\ArrowFunction;
use Lines202605\PhpParser\Node\Expr\AssignOp\Coalesce;
use Lines202605\PhpParser\Node\Expr\BinaryOp\Spaceship;
use Lines202605\PhpParser\Node\Expr\CallLike;
use Lines202605\PhpParser\Node\Expr\Match_;
use Lines202605\PhpParser\Node\Expr\NullsafeMethodCall;
use Lines202605\PhpParser\Node\Expr\NullsafePropertyFetch;
use Lines202605\PhpParser\Node\Expr\Throw_;
use Lines202605\PhpParser\Node\FunctionLike;
use Lines202605\PhpParser\Node\Identifier;
use Lines202605\PhpParser\Node\IntersectionType;
use Lines202605\PhpParser\Node\NullableType;
use Lines202605\PhpParser\Node\Param;
use Lines202605\PhpParser\Node\PropertyHook;
use Lines202605\PhpParser\Node\Stmt\Class_;
use Lines202605\PhpParser\Node\Stmt\ClassConst;
use Lines202605\PhpParser\Node\Stmt\Declare_;
use Lines202605\PhpParser\Node\Stmt\Enum_;
use Lines202605\PhpParser\Node\Stmt\Expression;
use Lines202605\PhpParser\Node\Stmt\Property;
use Lines202605\PhpParser\Node\UnionType;
use Lines202605\TomasVotruba\Lines\FeatureCounter\Enum\PhpVersion;
final class FeatureCollector
{
    ...
    public function __construct()
    {
        ...
        $this->phpFeatures[] = new PhpFeature(PhpVersion::PHP_70, 'Coalesce ?? operator', function (Node $node) : bool {
            return $node instanceof \Lines202605\PhpParser\Node\Expr\BinaryOp\Coalesce;
        });
        ...
        $this->phpFeatures[] = new PhpFeature(PhpVersion::PHP_80, 'Throw expression', function (Node $node) : bool {
            if ($node instanceof Expression && $node->expr instanceof Throw_) {
                $node->expr->setAttribute('is_throw_statement', \true);
                return \false;
            }
