...
abstract class Input implements RawInputInterface, StreamableInputInterface
{
    ...
    public function unparse(?array $optionNames = null) : array
    {
        ...
        $filteredRawOptions = null === $optionNames ? $rawOptions : \array_intersect_key($rawOptions, \array_fill_keys($optionNames, ''));
        $unparsedOptions = [];
        foreach ($filteredRawOptions as $optionName => $parsedOption) {
            $option = $this->definition->getOption($optionName);
            switch (\true) {
                case $option->isNegatable():
                    $unparsedOptions[] = [\sprintf('--%s%s', $parsedOption ? '' : 'no-', $optionName)];
                    break;
                case !$option->acceptValue():
                    $unparsedOptions[] = [\sprintf('--%s', $optionName)];
                    break;
                case $option->isArray():
                    $unparsedOptions[] = \array_map(static function ($item) use($optionName) {
                        return \sprintf('--%s=%s', $optionName, $item);
                    }, $parsedOption);
                    break;
                default:
                    $unparsedOptions[] = [\sprintf('--%s=%s', $optionName, $parsedOption)];
                    break;
            }
        }
